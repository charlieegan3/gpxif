...
func CheckLocalTime(imageFile string, g gpx.GPXDataset) ([]Operation, error) {
	...
	currentDateTime, err := exif.GetKeyString(imageFile, "DateTimeOriginal")
	if err != nil {
		return operations, fmt.Errorf("failed to get DateTimeOriginal: %v", err)
	}
	currentSubSecTime, err := exif.GetKeyString(imageFile, "SubSecTimeOriginal")
	if err != nil {
		return operations, fmt.Errorf("failed to get SubSecTimeOriginal: %v", err)
	}
	currentOffset, err := exif.GetKeyString(imageFile, "OffsetTimeOriginal")
	if err != nil {
